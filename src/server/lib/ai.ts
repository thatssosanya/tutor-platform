...
- Решение квадратных уравнений: При решении полных квадратных уравнений вида $ax^2 + bx + c = 0$, обязательно используй метод через дискриминант. Сначала вычисли значение дискриминанта по формуле $D = b^2 - 4ac$. Затем найди корни по формуле $x_{1,2} = \\dfrac{-b \\pm \\sqrt{D}}{2a}$.
...
В LaTeX сегментах обязательно используй функцию \\text для обычного текста. Используй запятую как десятичный разделитель и оборачивай ее в пару \`{}\`, иначе LaTeX отобразит после нее пробел.
Умножение обозначай функцией \\cdot. Деление обозначай функцией \\dfrac, например, \\dfrac{1}{2}. При делении дроби на дробь используй символ :.
...
export const SOLUTION_TYPE_INSTRUCTIONS: Record<string, string> = {
  [SolutionType.SHORT]:
    "Поле \`solution\` должно быть строкой, содержащей только одно число или слово. В поле \`solution\` дроби записывай в десятичной форме. Десятичный разделитель для поля \`solution\` — точка.",
  [SolutionType.LONG]:
...
export const EXAMPLES = {
  [SolutionType.SHORT]: [
    ...
    },
    {
      ...
      response: JSON.stringify({
        hint: "Представьте основание $24$ как произведение $8 \\cdot 3$. Затем примените свойства степеней: $(ab)^n = a^n b^n$ и $\\dfrac{a^m}{a^n} = a^{m-n}$.",
        work: "$\\dfrac{8^{10} \\cdot 3^{11}}{24^{9}} = \\dfrac{8^{10} \\cdot 3^{11}}{(8 \\cdot 3)^{9}} = \\dfrac{8^{10} \\cdot 3^{11}}{8^{9} \\cdot 3^{9}}$\n\n$= 8^{10-9} \\cdot 3^{11-9} = 8^{1} \\cdot 3^{2} = 8 \\cdot 9 = 72$",
        solution: "72",
      }),
    },
    {
      ...
      response: JSON.stringify({
        hint: "Вероятность противоположного события можно найти по формуле $P_{A} = 1 - P_\\bar{A}$. Сначала найдите вероятность купить сумку с дефектом.",
        work: "Найдём вероятность того, что купленная сумка имеет дефект:\n\n$P_{дефект} = \\dfrac{4}{50} = 0.08$\n\nВероятность купить сумку без дефекта — это вероятность противоположного события:\n\n$P_\\text{без дефекта} = 1 - P_{дефект} = 1 - 0.08 = 0.92$",
        solution: "0.92",
      }),
    },
  ],
  ...
  [SolutionType.MULTICHOICE]: [
    {
      ...
      response: JSON.stringify({
        ...
        work: "Приведём границы интервала к знаменателю 19:\n\n$3 = \\dfrac{3 \\cdot 19}{19} = \\dfrac{57}{19}$\n\n$4 = \\dfrac{4 \\cdot 19}{19} = \\dfrac{76}{19}$\n\nИнтервал $(3; 4)$ соответствует интервалу $(\\dfrac{57}{19}; \\dfrac{76}{19})$.\n\nСреди предложенных дробей только числитель дроби $\\dfrac{68}{19}$ удовлетворяет неравенству $57 < 68 < 76$.",
        solution: "3",
      }),
    },
    {
  ...
  [SolutionType.MULTIRESPONSE]: [
    ...
    },
    {
      ...
      response: JSON.stringify({
        hint: "1. Используйте формулу площади треугольника: $S = \\dfrac{1}{2}ab \\sin\\gamma$ и учтите диапазон значений синуса.\n\n2. Вспомните формулу средней линии трапеции.\n\n3. Вспомните признаки подобия треугольников.",
        work: "Проанализируем каждое утверждение:\n\n1. **Верно.** Площадь треугольника со сторонами $a$ и $b$ и углом $\\gamma$ между ними равна $S = \\dfrac{1}{2}ab\\sin\\gamma$. Поскольку $\\sin\\gamma \\le 1$, то $S \\le \\dfrac{1}{2}ab$. Так как стороны $a$ и $b$ положительны, $\\dfrac{1}{2}ab < ab$, следовательно, $S < ab$.\n\n2. **Неверно.** Средняя линия трапеции равна полусумме её оснований, а не сумме.\n\n3. **Верно.** Это первый признак подобия треугольников (по двум углам).",
        solution: "1|3",
      }),
    },
  ],
